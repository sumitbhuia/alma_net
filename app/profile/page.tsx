...
//     const userId = "some-uuid"; // Replace with actual user id logic
  ...
  const handleSave = async () => {
    ...
    try {
      const updates = {
        ...
        profile_photo_url: "", // Add the profile_photo_url property
      };

      if (selectedFile) {
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from("profile-photos")
          .upload(`public/${profile.id}/${selectedFile.name}`, selectedFile, {
            upsert: true,
          });
        ...
        const imageUrl = uploadData?.path
          ? supabase.storage.from("profile-photos").getPublicUrl(uploadData.path).data.publicUrl
          : null;
